...
import static org.cellery.CelleryUtils.getConfigMap;
...
public class CreateConfiguration extends BlockingNativeCallableUnit {

    @Override
    public void execute(Context context) {
        LinkedHashMap configurationMap = ((BMap) context.getNullableRefArgument(0)).getMap();
        ...
        try {
            ...
            context.setError(BLangVMErrors.createError(context, "Unable to create configMap in path " + targetFile));
        } catch (BallerinaException e) {
            context.setError(BLangVMErrors.createError(context, "Unable to deploy configMap from file " + targetFile));
        }
    }
}
